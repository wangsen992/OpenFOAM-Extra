...
)
{
    ...
    {
        if ( 
            isA<fixedFluxPressureInletOutletFvPatchScalarField>(pBf[patchi]))
        {
            refCast<fixedFluxPressureInletOutletFvPatchScalarField>
            (
                pBf[patchi]
            ).updateCoeffs
            (
...
{
    myConstrainPressure(p, rho, U, phiHbyA, rAU, NullMRF());
}
...
{
    myConstrainPressure(p, geometricOneField(), U, phiHbyA, rAU, MRF);
}
...
{
    myConstrainPressure(p, U, phiHbyA, rAU, NullMRF());
}
