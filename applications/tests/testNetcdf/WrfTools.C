...
autoPtr<polyMesh> meshFromNc(netCDF::NcFile& dataFile, Time& runTime)
{
    // Start parsing the coordinate systems of the result
      ...
      Info << "Print wrfInfo: " << nl 
           << "CEN_LON : " << wrfInfo.CEN_LON << nl
           << "CEN_LAT : " << wrfInfo.CEN_LAT << endl;
      ...
      std::cout << "Loading Z vals " << std::endl;
      {
        float* tmp_full_3d = new float[N_Time*N_bottom_top_stag*N_south_north*N_west_east];
        netCDF::NcVar PHB(dataFile.getVar("PHB"));
        PHB.getVar(tmp_full_3d);
      ...
      std::cout << "Loading Z vals " << std::endl;
        ...
        Info << "mesh cells: " << tmesh->cells().size() << endl;
        ...
        Info << "mesh write complete" << endl;

    Info << "before return tmesh." << endl;
