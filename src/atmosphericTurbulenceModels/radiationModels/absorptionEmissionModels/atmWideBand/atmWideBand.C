...
{
    defineTypeNameAndDebug(atmWideBand, 0);
...
Foam::radiationModels::absorptionEmissionModels::atmWideBand::atmWideBand
(
...
:
    ...
    lookUpTablePtr_(),
    thermo_(mesh.lookupObject<fluidThermo>(basicThermo::dictName+".air")),
    Yj_(nSpecies_),
    totalWaveLength_(0)
{
    label nBand = 0;
    ...
    forAllConstIter(HashTable<label>, speciesNames_, iter)
    {
        ...
        // ad hoc patch (+".air") in following lines. Fix by adding a phase
        // attribute in class
        else if (mesh.foundObject<volScalarField>(iter.key()+".air"))
        {
            Yj_.set(j, &mesh.lookupObjectRef<volScalarField>(iter.key()+".air"));
            specieIndex_[iter()] = 0;
            j++;
        }
        else
...
Foam::radiationModels::absorptionEmissionModels::atmWideBand::aCont
(
    const label bandi
) const
{
...
Foam::tmp<Foam::volScalarField>
Foam::radiationModels::absorptionEmissionModels::atmWideBand::eCont
(
    const label bandi
) const
{
...
void Foam::radiationModels::absorptionEmissionModels::atmWideBand::correct
(
    volScalarField& a,
