...
void canopyEnergyTransferModel<BaseCanopyModel>::correctEnergyTransfer()
{
    ...
    // This is the actual energy transfer update loop
    forAllConstIter(labelHashSet, BaseCanopyModel::canopyCells(), iter)
    {
        ...
        //- Radiative Transfer
          ...
          scalar Ruleaf = aleafCell * GCell;

          //- compute bulk leaf temperature change in each cell from energy balance
          scalar dleaf =  (Ruleaf - (FheConv + FheLeConv + EleafCell)) 
                          /  (CleafCell * rholeafCell * laleafCell * hleafCell);
        ...
        Tleaf_[iter.key()].value() = TleafCell + dleaf;
    }
}
